...
function delayAnimationPostorder(node: BinarySearchTreeNode, isPrintable = false) {
...
async function postOrderManually(node) {
    ...
    while (S.length != 0) {
        ...
        await this.delayAnimationPostorder(current, false);
        ...
        if (prev == null || prev.left == current || prev.right == current) {
            if (current.left != null) {
                ...
                S.pop();
                await delayAnimationPostorder(current, true);
