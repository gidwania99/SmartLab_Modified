...
function generateGameTree(lvl:number){
  root = null;
  randomNumber = Math.floor(Math.random() * 8) + 0;

  tree=GameTree[lvl-1][randomNumber];
...
let gameNodeList: BinarySearchTreeNode[] = [];

function listOfNodesGame(root): void {
  if (root == null)
    return;
  gameNodeList.push(root)

  if (root.left != null) {
    listOfNodesGame(root.left)
  }
  if (root.right != null) {
    listOfNodesGame(root.right)
  }
}
