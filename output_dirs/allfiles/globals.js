...
function resetTimerPreorder(isTimeOver = false) {
...
function resetTimerInorder(isTimeOver = false) {
...
function drawLeftSubtreeInstruction(node) {
    ...
    let width = canvasComponent.width() / 17.5;
    var bottom = getRightY(node.left);
    var pointY = (bottom.nodeCircle.getCoordinates()[1] + bottom.parent.nodeCircle.getCoordinates()[1]) / 2;
...
function drawRightSubtreeInstruction(node) {
    ...
    let width = canvasComponent.width() / 17.5;
    var bottom = getRightY(node.right);
    var pointY = (bottom.nodeCircle.getCoordinates()[1] + bottom.parent.nodeCircle.getCoordinates()[1]) / 2;
...
function highlightNode(node) {
    ...
    var pointY = (node.nodeCircle.getCoordinates()[1]);
    //let pointX=node.right.nodeCircle.x+hieght;
...
function removeNodeFromAnswerList(value) {
    for (var i = 0; i < userAnsList.length; i++) {
